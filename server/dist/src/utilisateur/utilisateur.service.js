...
let UtilisateurService = class UtilisateurService {
    ...
    async findOne(id) {
        ...
        if (!user) {
            throw new common_1.NotFoundException(`Utilisateur avec ID ${id} introuvable`);
        }
        return user;
    }
    async create(data) {
        ...
        if (existing) {
            throw new common_1.ConflictException(`Un utilisateur avec l'email ${data.email} existe déjà`);
        }
    ...
    async remove(id) {
        ...
        if (user.role === 'ADMIN') {
            ...
            if (adminCount <= 1) {
                throw new common_1.ConflictException("Impossible de supprimer le dernier administrateur.");
            }
        }
